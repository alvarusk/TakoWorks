...
def parse_json_translations(raw_content: str, fallback_lines: List[str]) -> List[str]:
    """
    Intenta extraer un JSON del tipo:
      {"translations": ["...", "...", ...]}
    Si el JSON viene “sucio” (fences, texto extra, o saltos de línea literales dentro
    de strings), aplica heurísticas para rescatar el array.

    En caso de fallo, devuelve fallback_lines (normalmente el chunk original).
    """
    ...
    # 4) Parse “tolerante” (para casos tipo: Unterminated string / newlines literales)
    try:
